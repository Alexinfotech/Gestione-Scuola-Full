...
public class ProvaCodiceFiscale extends HttpServlet {
    ...
    private String costruisciUrlApi(String cognome, String nome, String sesso, String comuneDiNascita, String provincia, String dataNascita) throws UnsupportedEncodingException {
        return "http://api.miocodicefiscale.com/calculate?" +
               "lname=" + encodeURIComponent(cognome) +
               "&fname=" + encodeURIComponent(nome) +
               "&gender=" + encodeURIComponent(sesso) +
               "&city=" + encodeURIComponent(comuneDiNascita) +
               "&state=" + encodeURIComponent(provincia) +
               "&day=" + encodeURIComponent(dataNascita.split("-")[2]) +
               "&month=" + encodeURIComponent(dataNascita.split("-")[1]) +
               "&year=" + encodeURIComponent(dataNascita.split("-")[0]) +
               "&omocodia_level=1" +
