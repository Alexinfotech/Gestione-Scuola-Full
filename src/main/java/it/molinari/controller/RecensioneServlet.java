...
public class RecensioneServlet extends HttpServlet {
		...
		try {
			...
			case "listRecensioneProdotto":
				...
				if (id != null && !id.trim().isEmpty()) {
					...
					System.out.println(
							"Richiesta URL: " + request.getRequestURL().toString() + "?" + request.getQueryString());
			...
			/*
			 * case "search": String testo = request.getParameter("testo"); RecensioneDTO
			 * recensione = null; try { List<RecensioneDTO> listaRecensioni =
			 * gestioneRecensione.recupera(); for (RecensioneDTO p : listaRecensioni) { if
			 * (p.getTesto().equals(testo)) { recensione = p; break; } } if (recensione !=
			 * null) { request.setAttribute("prodotto", recensione);
			 * request.getRequestDispatcher("views/recensione/modificaRecensione.jsp").
			 * forward(request, response); } else { request.setAttribute("errore",
	 ...
	 * 
	 * try { if ("create".equals(action)) { RecensioneDTO recensione =
	 * creaRecensioneDTO(request); gestioneRecensione.create(recensione);
	 * response.sendRedirect("RecensioneServlet?action=list"); } else if
	 * ("update".equals(action)) { // Codice omesso per brevità } else {
	 * request.setAttribute("errore", "Azione non supportata.");
	 * request.getRequestDispatcher("views/errore.jsp").forward(request, response);
	 * } } catch (SQLException e) { e.printStackTrace();
	 * request.setAttribute("errore", "Errore durante l'accesso al database: " +
	 * e.getMessage());
	 * request.getRequestDispatcher("views/errore.jsp").forward(request, response);
		...
		try {
			...
			if (prodottoIdStr != null && !prodottoIdStr.isEmpty()) {
				try {
					...
					System.out.println("Tipo di ID Prodotto: Integer"); // Stampiamo il tipo di ID del prodotto
																		// (Integer)
					recensione.setProdottoId(prodottoId);
				} catch (NumberFormatException ex) {
					System.out.println("Tipo di ID Prodotto: Stringa"); // Stampiamo il tipo di ID del prodotto
																		// (Stringa)
					recensione.setProdottoId(null); // Imposta a null in caso di errore di conversione
				}
			} else {
				System.out.println("ID Prodotto non fornito o vuoto."); // Aggiungi la stampa per gestire il caso in cui
																		// idProdotto sia null o vuoto
				// Puoi lanciare un'eccezione, impostare un valore predefinito o fare altro
				// Ad esempio, potresti impostare il valore di prodottoId a 0 o a un altro
				// valore predefinito
			...
			e.printStackTrace();
			// Puoi scegliere di lanciare un'eccezione qui o gestire l'errore in modo
			// diverso
		}
