...
public class ProdottoDAO extends Dao implements DaoInterface<ProdottoDTO> {
	...
	public void update(ProdottoDTO prodotto) throws SQLException {
		...
		try (Connection conn = getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
			stmt.setString(1, prodotto.getNomeProdotto());
			stmt.setString(2, prodotto.getPrezzo());
			// Assicurati che il metodo getIva() restituisca una stringa, altrimenti
			// potrebbe essere necessario convertirlo
			stmt.setString(3, prodotto.getIva());
			stmt.setString(4, prodotto.getDescrizioneProdotto());
			// Assicurati che getQuantita() restituisca una stringa. Se restituisce un tipo
			// numerico (es. Integer), usa stmt.setInt() o stmt.setObject()
			stmt.setString(5, prodotto.getQuantita());
	...
	/*
	 * public void acquista(ProdottoDTO prodotto) throws SQLException { // Ottiene
	 * l'ID del prodotto dall'oggetto ProdottoDTO int prodottoId = prodotto.getId();
	 * 
	 * // Prima recupera l'attuale quantità del prodotto String sqlGetQuantita =
	 * "SELECT quantita FROM prodotto WHERE id = ?"; int quantita = 0; try
	 * (Connection conn = getConnection(); PreparedStatement stmt =
	 * conn.prepareStatement(sqlGetQuantita)) { // Utilizza prodottoId per impostare
	 * il parametro della query stmt.setInt(1, prodottoId); try (ResultSet resultSet
	 * = stmt.executeQuery()) { if (resultSet.next()) { quantita =
	 * resultSet.getInt("quantita"); } } }
	 * 
	 * // Controlla se la quantità è maggiore di 0 per evitare quantità negative if
	 * (quantita > 0) { // Decrementa la quantità di 1 quantita--;
	 * 
	 * // Aggiorna la quantità nel database String sqlUpdateQuantita =
	 * "UPDATE prodotto SET quantita = ? WHERE id = ?"; try (Connection conn =
	 * getConnection(); PreparedStatement stmt =
	 * conn.prepareStatement(sqlUpdateQuantita)) { stmt.setInt(1, quantita);
	 * stmt.setInt(2, prodottoId); stmt.executeUpdate(); } } else { throw new
	 * SQLException("Quantità insufficiente per l'acquisto."); } }
	 */
	public void acquista(ProdottoDTO prodotto, int idUtente) throws SQLException {
		...
		if (quantita > 0) {
			...
			try (Connection conn = getConnection(); PreparedStatement stmt = conn.prepareStatement(sqlInsertOrdine)) {
				...
				// Supponiamo che l'ID dell'utente sia disponibile come costante o da un altro
				// luogo.
