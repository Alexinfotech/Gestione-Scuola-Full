...
public class CarrelloDAO extends Dao implements DaoInterface<ProdottoDTO> {
	...
	/*
	 * public void create(int loginId, int prodottoId, int quantita) throws
	 * SQLException { String sql =
	 * "INSERT INTO carrello (login_id, prodotto_id, quantita) VALUES (?, ?, ?)";
	 * try (Connection conn = getConnection(); PreparedStatement stmt =
	 * conn.prepareStatement(sql)) { stmt.setInt(1, loginId); stmt.setInt(2,
	 * prodottoId); stmt.setInt(3, quantita); stmt.executeUpdate(); } }
	 */
